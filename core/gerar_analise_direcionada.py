...
def calcular_metricas_direcionadas(grafo_dir):
    ...
    fontes = [
        no for no in grafo_dir.nodes() if out_degrees[no] > 0 and in_degrees[no] == 0
    ]

    # Identificar sumidouros (in-degree > 0, out-degree = 0)
    sumidouros = [
        no for no in grafo_dir.nodes() if in_degrees[no] > 0 and out_degrees[no] == 0
    ]
...
def calcular_metricas_nao_direcionadas(grafo):
    ...
    print("   Calculando betweenness não-direcionado...")
    from tqdm import tqdm
    ...
    return {"degree": degrees, "betweenness": betweenness, "closeness": closeness}
...
def comparar_metricas(metricas_dir, metricas_nao_dir, papeis_dict):
    ...
    # Top por PageRank
    top_pagerank = sorted(
        metricas_dir["pagerank"].items(), key=lambda x: x[1], reverse=True
    )[:20]
    nos_analisar.update([no for no, _ in top_pagerank])

    # Top por betweenness direcionado
    top_betw_dir = sorted(
        metricas_dir["betweenness"].items(), key=lambda x: x[1], reverse=True
    )[:20]
    nos_analisar.update([no for no, _ in top_betw_dir])

    # Top por out-degree
    top_out = sorted(
        metricas_dir["out_degree"].items(), key=lambda x: x[1], reverse=True
    )[:20]
        ...
        if nos_papel_int:
            stats_por_papel[papel] = {
                ...
                "in_degree_medio": round(
                    sum(metricas_dir["in_degree"][n] for n in nos_papel_int)
                    / len(nos_papel_int),
                    2,
                ),
                "out_degree_medio": round(
                    sum(metricas_dir["out_degree"][n] for n in nos_papel_int)
                    / len(nos_papel_int),
                    2,
                ),
                "pagerank_medio": round(
                    sum(metricas_dir["pagerank"][n] for n in nos_papel_int)
                    / len(nos_papel_int),
                    6,
                ),
            }
...
def main():
    ...
    comparacoes = comparar_metricas(metricas_dir, metricas_nao_dir, papeis_dict)
    ...
    stats_fluxo = gerar_estatisticas_fluxo(grafo_direcionado, metricas_dir, papeis_dict)
    ...
    resultado = {
        ...
        "estatisticas_gerais": {
            ...
            - len(metricas_dir["fontes"])
            - len(metricas_dir["sumidouros"]),
        },
    ...
    print("=" * 80)
    print("\n📊 Estrutura de Fluxo:")
    print(f"   • Fontes (out only): {resultado['estatisticas_gerais']['num_fontes']}")
    print(
