...
def inferir_papel_no(grafo: nx.Graph, no: int, grau: int, betweenness: float,
                     clustering: float, eh_articulacao: bool, 
                     eh_hub_comunidade: bool, comunidade_id: int) -> str:
    """
...
def classificar_nos(grafo: nx.Graph, analise_comunidades: Dict, 
                    analise_criticidade: Dict) -> Dict[int, Dict]:
...
def main():
    ...
    analise_criticidade = carregar_criticidade('../ui/public/analise_criticidade.json')
    
    # Classifica nós
    print("\n🏷️ Classificando nós por papel...")
    classificacao = classificar_nos(grafo, analise_comunidades, analise_criticidade)
    ...
    for papel, stats in estatisticas['estatisticas'].items():
        ...
        print(f"  - Grau médio: {stats['grau_medio']:.2f} (min: {stats['grau_min']}, max: {stats['grau_max']})")
