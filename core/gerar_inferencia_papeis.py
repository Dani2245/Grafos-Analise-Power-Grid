...
def inferir_papel_no(
    grau: int,
    betweenness: float,
    clustering: float,
    eh_articulacao: bool,
    eh_hub_comunidade: bool,
) -> str:
    """
...
def classificar_nos(grafo: nx.Graph, analise_comunidades: Dict) -> Dict[int, Dict]:
    ...
    print("🔍 Carregando métricas de analise_criticidade.json...")

    # Carregar betweenness e pontos de articulação (OTIMIZADO - não recalcula)
    try:
        analise_crit = carregar_criticidade("../ui/public/analise_criticidade.json")
        betweenness = {
            int(k): v
            for k, v in analise_crit["centralidade_intermediacao"]["todos_nos"].items()
        }
        pontos_articulacao = set(analise_crit["pontos_articulacao"]["lista_completa"])
        print(
            "  ✓ Betweenness e pontos de articulação carregados (economia: ~3 minutos)"
        )
    except Exception as e:
        print(f"  ⚠️  Erro ao carregar analise_criticidade.json: {e}")
    ...
    # Clustering coefficient (ainda precisa calcular - não está completo no JSON)
...
def listar_top_nos_por_papel(
    classificacao: Dict[int, Dict], papel: str, top_n: int = 20
) -> List[Dict]:
    """Lista os top N nós de um determinado papel"""
    nos_papel = [
        (no, info) for no, info in classificacao.items() if info["papel"] == papel
    ]
...
def main():
    ...
    analise_comunidades = carregar_comunidades("../ui/public/analise_comunidades.json")
    ...
    top_transformadores = listar_top_nos_por_papel(classificacao, "TRANSFORMADOR", 20)
    top_linhas = listar_top_nos_por_papel(classificacao, "LINHA_TRANSMISSAO", 20)
    ...
    classificacao_serializable = {str(no): info for no, info in classificacao.items()}
    ...
    for papel, stats in estatisticas["estatisticas"].items():
        print(f"\n{papel}:")
        print(f"  - Quantidade: {stats['quantidade']} ({stats['percentual']:.1f}%)")
        print(
