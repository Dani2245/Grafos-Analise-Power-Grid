...
def calcular_metricas_normalizadas_original(G: nx.Graph) -> Dict:
    ...
    # Métricas de centralidade (amostrar para performance)
    print("   Calculando betweenness (pode demorar)...")
...
def calcular_metricas_normalizadas_novo(G: nx.DiGraph) -> Dict:
    ...
    fontes = [n for n in G.nodes() if out_degrees[n] >
              0 and in_degrees[n] == 0]
    sumidouros = [n for n in G.nodes() if in_degrees[n] >
                  0 and out_degrees[n] == 0]
    ...
    comparacao = {
        ...
        'robustez': {
            'taxa_pontos_articulacao': {
                'dataset_original': metricas_original.get('taxa_pontos_articulacao', 'N/A'),
...
def main():
    ...
    print("\n[2/5] Calculando métricas do dataset original...")
    metricas_original = calcular_metricas_normalizadas_original(G_original)

    print("\n[3/5] Calculando métricas do novo dataset...")
    metricas_novo = calcular_metricas_normalizadas_novo(G_novo)

    print("\n[4/5] Gerando tabela comparativa...")
    comparacao = gerar_tabela_comparativa(metricas_original, metricas_novo)

    print("\n[5/5] Gerando índices normalizados...")
