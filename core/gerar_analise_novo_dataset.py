...
def gerar_series_temporais(df: pd.DataFrame, amostragem: int = 10) -> Dict:
    ...
    series = {
        "timestamps": df_amostrado["timestamp"]
...
def calcular_metricas_direcionadas(df: pd.DataFrame) -> Dict:
    ...
    fontes = [no for no in G.nodes() if out_degrees[no] > 0 and in_degrees[no] == 0]
    sumidouros = [no for no in G.nodes() if in_degrees[no] > 0 and out_degrees[no] == 0]
...
def analisar_falhas(df: pd.DataFrame) -> Dict:
    ...
    correlacao_falha_instabilidade = df["fault_detected"].corr(df["grid_status"])
    ...
    comparacao_falha = {
        "com_falha": {
            ...
            "taxa_instabilidade": (
                round(
                    timestamps_com_falha["grid_status"].sum()
                    / len(timestamps_com_falha)
                    * 100,
                    2,
                )
                if len(timestamps_com_falha) > 0
                else 0
            ),
        },
        "sem_falha": {
            ...
            "taxa_instabilidade": (
                round(
                    timestamps_sem_falha["grid_status"].sum()
                    / len(timestamps_sem_falha)
                    * 100,
                    2,
                )
                if len(timestamps_sem_falha) > 0
                else 0
            ),
        },
    }
    ...
    comparacao_estabilidade = {
        "estavel": {
            ...
            "taxa_falhas": (
                round(
                    timestamps_estavel["fault_detected"].sum()
                    / len(timestamps_estavel)
                    * 100,
                    2,
                )
                if len(timestamps_estavel) > 0
                else 0
            ),
        },
        "instavel": {
            ...
            "taxa_falhas": (
                round(
                    timestamps_instavel["fault_detected"].sum()
                    / len(timestamps_instavel)
                    * 100,
                    2,
                )
                if len(timestamps_instavel) > 0
                else 0
            ),
        },
    }

    print(f"   ✓ Correlação fault/instabilidade: {correlacao_falha_instabilidade:.4f}")
...
def analise_estratificada_por_status(df: pd.DataFrame) -> Dict:
    ...
    if len(df_estavel) > 0:
        ...
        metricas_estavel = {
            ...
            "grau_medio": round(
                sum(dict(G_estavel.degree()).values()) / G_estavel.number_of_nodes(), 2
            ),
        }
    ...
    if len(df_instavel) > 0:
        ...
        metricas_instavel = {
            ...
            "grau_medio": round(
                sum(dict(G_instavel.degree()).values()) / G_instavel.number_of_nodes(),
                2,
            ),
        }
    ...
    return {"estavel": metricas_estavel, "instavel": metricas_instavel}
