    ...
    *** IMPORTANTE: LIMITAÇÃO METODOLÓGICA ***
    ...
    for idx, comunidade in enumerate(
        ...
        if nos_com_pos:
            ...
            distancias = [
                np.sqrt((pos[n][0] - centroide_x) ** 2 + (pos[n][1] - centroide_y) ** 2)
    ...
    for i in range(len(centroides_comunidades)):
        for j in range(i + 1, len(centroides_comunidades)):
            ...
            if c1["centroide_x"] is not None and c2["centroide_x"] is not None:
                dist = np.sqrt(
                    (c1["centroide_x"] - c2["centroide_x"]) ** 2
...
def gerar_analise_json(
    comunidades,
    modularidade,
    analise_comunidades,
    conexoes_intercomunidades,
    grupos_consumidores,
    ...
    dados = {
        "estatisticas_gerais": {
            ...
            "interpretacao_modularidade": (
                "ALTA (>0.7) - Comunidades muito bem definidas"
                if modularidade > 0.7
                else (
                    "MÉDIA (0.4-0.7) - Comunidades razoavelmente definidas"
                    if modularidade > 0.4
            ...
            "tamanho_medio_comunidade": round(
                ...
                / len(analise_comunidades),
                1,
            ),
            "maior_comunidade": max(analise_comunidades, key=lambda x: x["tamanho"])[
                "tamanho"
            ],
            "menor_comunidade": min(analise_comunidades, key=lambda x: x["tamanho"])[
                "tamanho"
            ],
        },
...
def main():
    ...
    for i, comunidade in enumerate(analise_comunidades[:5], 1):
        print(
            f"   {i}. Comunidade {comunidade['id']}: {comunidade['tamanho']} nós "
