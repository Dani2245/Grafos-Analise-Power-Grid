...
def analisar_comunidades(grafo, comunidades):
    ...
    for idx, comunidade in enumerate(comunidades):
        ...
        nos_ordenados = sorted(comunidade, key=lambda x: grafo.degree(x), reverse=True)
        top_hubs = [
        ...
        try:
            pontos_articulacao_internos = list(nx.articulation_points(subgrafo))
        except:
...
def identificar_grupos_consumidores(grafo, comunidades):
    ...
    for idx, comunidade in enumerate(comunidades):
        consumidores_na_comunidade = [n for n in comunidade if n in consumidores]
        ...
        if consumidores_na_comunidade:
            ...
            for consumidor in consumidores_na_comunidade:
                try:
                    dist = nx.shortest_path_length(grafo, consumidor, hub_principal)
                    distancias.append(dist)
                except:
                    pass
            
            distancia_media = sum(distancias) / len(distancias) if distancias else 0
            ...
            grupos_consumidores.append({
                ...
                "consumidores": sorted(consumidores_na_comunidade)[:100],  # Limitar a 100
...
def gerar_analise_json(grafo, comunidades, modularidade, analise_comunidades, 
    ...
    dados = {
        "estatisticas_gerais": {
            ...
            "tamanho_medio_comunidade": round(
                sum(c['tamanho'] for c in analise_comunidades) / len(analise_comunidades), 1
            ),
...
def main():
    ...
    conexoes_intercomunidades = analisar_conexoes_intercomunidades(grafo, comunidades)
    ...
    dados = gerar_analise_json(
        grafo, comunidades, modularidade, analise_comunidades,
        conexoes_intercomunidades, grupos_consumidores
    )
    ...
    print(f"   • Número de comunidades: {dados['estatisticas_gerais']['num_comunidades']}")
    print(f"   • Modularidade: {dados['estatisticas_gerais']['modularidade']}")
    print(f"   • Interpretação: {dados['estatisticas_gerais']['interpretacao_modularidade']}")
    
    print("\n🏘️ Tamanho das Comunidades:")
    print(f"   • Tamanho médio: {dados['estatisticas_gerais']['tamanho_medio_comunidade']:.1f} nós")
    print(f"   • Maior comunidade: {dados['estatisticas_gerais']['maior_comunidade']} nós")
    print(f"   • Menor comunidade: {dados['estatisticas_gerais']['menor_comunidade']} nós")
    
    print("\n🔗 Conexões entre Comunidades:")
    print(f"   • Total de conexões: {dados['conexoes_intercomunidades']['total_conexoes']} pares")
    print(f"   • Arestas intercomunidades: {dados['conexoes_intercomunidades']['total_arestas_intercomunidades']}")
    print(f"   • Percentual: {dados['conexoes_intercomunidades']['percentual_arestas_intercomunidades']}%")
    
    print("\n👥 Grupos de Consumidores:")
    print(f"   • Total de consumidores: {dados['grupos_consumidores']['total_consumidores']}")
    print(f"   • Percentual da rede: {dados['grupos_consumidores']['percentual_rede']}%")
    print(f"   • Distribuídos em: {len(dados['grupos_consumidores']['grupos'])} comunidades")
